...
export const parseBrandTalkSendBody = (input: unknown): ValidationResult<SendBrandTalkBodyDto> => {
  const source =
  ...
  if (source.reservedTime !== undefined) {
    issues.push({ field: "reservedTime", reason: "reservedTime is not allowed in brandtalk send endpoint" });
  }
