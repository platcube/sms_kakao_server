...
import {
  ...
  sendPrcompanyBrandTalkImmediate,
} from "@/libs/integrations/prcompany/prcompany.brandtalk";
import { prisma } from "@/libs/prisma/client";

const DEFAULT_MAX_RETRY = 3;

const isRetryableBrandTalkCode = (code: number) => {
...
const buildProviderPayload = (input: SendBrandTalkBodyDto) => ({
...
// 카카오 브랜드메시지(구 친구톡) 즉시 발송
export const sendBrandTalkMessage = async (input: SendBrandTalkBodyDto): Promise<SendBrandTalkResponseDto> => {
  ...
  try {
    const providerResponse = await sendPrcompanyBrandTalkImmediate({
      callback: input.senderPhone,
      phones: input.recipientPhone,
      title: input.title,
      message: input.message,
...
const handleProviderResponse = async (args: {
  ...
  return toResponseDto({
    ...
    reason: {
      ...
      message: providerResponse.ResMsg ?? "카카오 브랜드메시지 발송에 실패했습니다.",
    },
  });
};
